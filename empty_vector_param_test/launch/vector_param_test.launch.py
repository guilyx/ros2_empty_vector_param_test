...
class RewrittenYaml(launch.Substitution):  # type: ignore[misc]
    ...
    def substitute_params(
        self, yaml: dict[str, YamlValue], param_rewrites: dict[str, str]
    ) -> None:
    ...
    def substitute_keys(
        self, yaml: dict[str, YamlValue], key_rewrites: dict[str, str]
    ) -> None:
...
def generate_launch_description():
    ...
    pkg_share = get_package_share_directory("empty_vector_param_test")
    params = os.path.join(pkg_share, "config", "params.yaml")

    # Declare the launch arguments
    config_file = LaunchConfiguration("config_file", default=params)
    test_param = LaunchConfiguration("node_name", default="test_substituted")
    namespace = LaunchConfiguration("namespace", default="ns")

    # Create parameter substitutions
    substitutions = {"test_param": test_param}

    parsed_config = RewrittenYaml(
    ...
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                "namespace",
                default_value="test_namespace",
                description="Namespace for the node",
            ),
            Node(
                package="empty_vector_param_test",
                executable="vector_param_node",
                namespace=namespace,
